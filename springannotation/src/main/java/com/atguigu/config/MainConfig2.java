 ...
 * @Conditional 任务7 @Conditional-按照条件注册bean
 *              配在类上,类中组件统一设置,则需要满足这个条件,这个类中配置的所有bean注册才能生效
 * 任务8 @Import导入组件,id默认是组件的全类名
 * 任务9 @Import-使用ImportSelector, 这里MyImportSelector就是实现类
 * 任务10 @Import-使用ImportBeanDefinitionRegistrar, 这里 MyImportBeanDefinitionRegistrar为实现类
 */
...
//    @Scope("prototype")
     ...
     * 		2）、ImportSelector:返回需要导入的组件的全类名数组； 任务9
     * 		3）、ImportBeanDefinitionRegistrar:手动注册bean到容器中  任务10
     * 4）、使用Spring提供的 FactoryBean（工厂Bean）; 任务11
