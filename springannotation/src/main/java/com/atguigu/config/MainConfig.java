...
/**
 * 一个配置类   等于过去的配置文件
 * @Configuration 任务2 告诉spring这是一个配置类
 * @ComponentScan 任务3 包扫描,只要标注了@Controller,@Service,@Repositiry,@Component任何一个的组件,都会被自动扫描加入容器中
 *                  与xml配置方式中context:component-scan的一致
 *                  value:指定要扫描的包
 *                  excludeFilters = Filter[] 指定扫描时按照什么规则需要排除掉的组件,
 *                                      过滤规则有ANNOTATION 按照注解, ASSIGNABLE_TYPE 按照给定的类型, 前两个常用
 *                                      ASPECTJ ASPECTJ表达式类型, REGEX正则,
 *                                      CUSTOM自定义规则,看注解说明发现需要有TypeFilter的实现类,自定义的在任务4讲
 ...
 *                  @ComponentScan点进去还可以发现有 @Repeatable(ComponentScans.class) 说明这是个重复注解,
